...
//  TODO DONE(1) Implement the delete method of the ContentProvider
    /**
...
//          TODO DONE(2) Only implement the functionality, given the proper URI, to delete ALL rows in the weather table
        if (selection == null)
            selection = "1";

        SQLiteDatabase weatherDb = mOpenHelper.getWritableDatabase();

        int numOfDeletedRows = 0;

        switch (sUriMatcher.match(uri)) {
            case CODE_WEATHER:
                numOfDeletedRows = weatherDb.delete(WeatherContract.WeatherEntry.TABLE_NAME, selection, selectionArgs);
                break;
            default:
                throw new UnsupportedOperationException("Unsupported URI: " + uri);
        }

//      TODO DONE(3) Return the number of rows deleted
