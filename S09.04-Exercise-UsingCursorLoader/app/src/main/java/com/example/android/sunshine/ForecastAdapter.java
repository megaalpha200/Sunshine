...
//      TODO DONE(6) Move the cursor to the appropriate position
        boolean dataFound = mCursor.moveToPosition(position);
//      TODO DONE(7) Generate a weather summary with the date, description, high and low
        long dateMills = mCursor.getLong(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_DATE));
        String dateStr = SunshineDateUtils.getFriendlyDateString(mContext, dateMills, false);

        int weatherID = mCursor.getInt(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID));
        String weatherDes = SunshineWeatherUtils.getStringForWeatherCondition(mContext, weatherID);

        double tempLowInCelcius = mCursor.getDouble(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP));
        double tempHighInCelcius = mCursor.getDouble(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP));
        String highLowTempFormated = SunshineWeatherUtils.formatHighLows(mContext, tempHighInCelcius, tempLowInCelcius);

        String weatherSummary = dateStr + " - " + weatherDes + " - " + highLowTempFormated;
...
//  TODO DONE(11) Create a new method that allows you to swap Cursors.
    ...
    class ForecastAdapterViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View v) {
            //  TODO DONE(13) Instead of passing the String from the data array, use the weatherSummary text!
